...
struct Camera {
	...
	struct CameraHash {
		size_t operator()(const Camera& camera) const {
			...
			seed ^= std::hash<uint32_t>()(camera.width) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
			seed ^= std::hash<uint32_t>()(camera.height) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
			for (REAL p: camera.params)
				seed ^= std::hash<REAL>()(p) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
			return seed;
		}
	};
